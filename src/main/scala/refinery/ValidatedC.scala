...
object ValidatedC {
  ...
  implicit def applicative[C, E]: Applicative[ValidatedC[C, E, *]] = new Applicative[ValidatedC[C, E, *]] {
    override def pure[A](x: A): ValidatedC[C, E, A] = ValidatedC.Valid(Chain.empty[C], x)

    override def ap[A, B](ff: ValidatedC[C, E, A => B])(fa: ValidatedC[C, E, A]): ValidatedC[C, E, B] = (ff, fa) match {
